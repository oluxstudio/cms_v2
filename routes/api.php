...
Route::post('/sites/{siteName}/block-form/{blockId}', [\App\Http\Controllers\Api\BlockFormController::class, 'store'])->middleware(['throttle:leads', 'site.origin', 'honeypot'])->name('api.block-form');
...
Route::post('/sites/{siteName}/posts/{slug}/view',  [\App\Http\Controllers\Api\PostApiController::class, 'view'])->middleware('throttle:engagement')->name('api.posts.view');
Route::post('/sites/{siteName}/posts/{slug}/like',  [\App\Http\Controllers\Api\PostApiController::class, 'like'])->middleware('throttle:engagement')->name('api.posts.like');
...
Route::post('/sites/{siteName}/estimator/request', [\App\Http\Controllers\Api\EstimatorController::class, 'store'])->middleware(['throttle:leads', 'site.origin', 'honeypot'])->name('api.estimator.request');
...
Route::post('/sites/{siteName}/quote/request', [\App\Http\Controllers\Api\EstimatorController::class, 'store'])->middleware(['throttle:leads', 'site.origin', 'honeypot'])->name('api.quote.request');
// Submit plain interest ("I'm interested") — Contact + notification + owner email.
Route::post('/sites/{siteName}/interest', [\App\Http\Controllers\Api\InterestController::class, 'store'])->middleware(['throttle:leads', 'site.origin', 'honeypot'])->name('api.interest');
...
Route::middleware(['auth.token', 'throttle:token-api'])->group(function () {
    ...
    Route::get('/me', function (\Illuminate\Http\Request $request) {
        ...
        return response()->json([
            ...
            'sites' => $token->site
                ? [$token->site->name]
                : \App\Models\Site::all()->filter(fn ($s) => $s->accessibleBy($user))->pluck('name')->values(),
        ]);
    })->name('api.me');
