...
trait HasFieldSchema
{
    ...
    public function buildValidationRules(): array
    {
        ...
        foreach ($this->fields ?? [] as $field) {
            ...
            if ($type === 'email')  $fieldRules[] = 'email:rfc';
            if ($type === 'url')    $fieldRules[] = 'url';
            if ($type === 'number') $fieldRules[] = 'numeric';
            if ($type === 'date')   $fieldRules[] = 'date';
            if ($type === 'tel')    $fieldRules[] = 'regex:/^[\+\d\s\-\(\)\.]{6,25}$/';

            if (in_array($type, ['select', 'radio']) && ! empty($options)) {
                $fieldRules[] = 'in:' . implode(',', $options);
            }

            if ($min !== null && $min !== '') $fieldRules[] = 'min:' . $min;
            if ($max !== null && $max !== '') $fieldRules[] = 'max:' . $max;
    ...
    public static function fieldValidationSummary(array $field): string
    {
        ...
        if ($field['required'] ?? false) $parts[] = 'Required';
        ...
        if (! empty($field['min'])) $parts[] = "Min: {$field['min']}";
        if (! empty($field['max'])) $parts[] = "Max: {$field['max']}";
