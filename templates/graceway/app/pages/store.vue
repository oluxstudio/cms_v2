<script setup lang="ts">
import SiteHeader from '~/components/SiteHeader.vue'
import PageHeroContent from '~/components/PageHeroContent.vue'
import BooksBlock from '~/components/BooksBlock.vue'
import BookStoreBlock from '~/components/BookStoreBlock.vue'
import SiteFooter from '~/components/SiteFooter.vue'

useHead({ title: 'Store — CAC Blackburn' })

// Blocks render in the CMS-configured order (original order as fallback).
const oluxBlocks: Record<string, any> = { 'site-header': SiteHeader, 'page-hero-content': PageHeroContent, 'books': BooksBlock, 'book-store': BookStoreBlock, 'site-footer': SiteFooter }
const oluxPage = useOluxPageOrder('/store', oluxBlocks)
</script>

<template>
  <div>
    <div id="preloader"></div>
    <component :is="b.comp" v-for="(b, i) in oluxPage" :key="`${b.key}-${i}`" :data-olx-key="b.key" data-olx-kind="component" />
  </div>
</template>
