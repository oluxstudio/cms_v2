# Deploying hairco.oluxstudio.com (containerized, Traefik edge)
...
and rsyncs it into `/srv/sites/hairco/public` — the container serves the
...
mkdir -p /srv/sites/hairco/public
cp deploy/vps/site-compose.yml /srv/sites/hairco/docker-compose.yml
cp deploy/vps/site-nginx.conf  /srv/sites/hairco/nginx.conf
cd /srv/sites/hairco && docker compose up -d
```
...
| `VPS_USER` | root (or a deploy user that owns /srv/sites/hairco/public) |
...
## Verify
`gh run watch -R oluxstudio/hairco` → `curl -I https://hairco.oluxstudio.com`
