...
export default defineNuxtPlugin(() => {
  ...
  style.textContent = `
    ...
    .olx-hot .olx-add-item,[data-olx-key]:hover .olx-add-item{opacity:1;pointer-events:auto}
  ...
  document.addEventListener('click', (e) => {
    ...
    // Links navigate — ask the CMS to switch its page selector so the iframe
    // reloads with site + edit state intact (a raw SPA navigation would drop
    // the ?site= and ?olx-edit= params). External links are inert in edit mode.
    ...
    if (a) {
      ...
      if (path) post({ type: 'olx-navigate', path })
      return
    }
