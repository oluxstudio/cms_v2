...
// First-paint gate: pages hold rendering until the CMS content for the
// rendering site is in (so visitors never see fallback text flash). For the
// template's own baked site the build-time snapshot IS current CMS content,
// so the gate opens immediately; other sites (preview / instant domains)
// wait for the live fetch, with a safety timeout so an unreachable CMS
// still renders fallbacks rather than a blank page.
...
export function useCms() {
  ...
  if (typeof window !== 'undefined' && !loaded) {
    const { site, baked } = useOluxSite()
    if (site === baked) ready.value = true // snapshot already carries this site
    loaded = pullCms().finally(() => { ready.value = true })
    setTimeout(() => { ready.value = true }, 4000) // CMS unreachable → fallbacks
  ...
  return { cms: data, field, items, ready }
}
