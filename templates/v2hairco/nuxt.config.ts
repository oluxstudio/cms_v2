  ...
  try {
    const res = await fetch(`${CMS}/api/site/connect-token`, {
      headers: {
        ...
        'X-Olux-Site': process.env.OLUX_SITE || 'v2-hairco',
    ...
    // A site-scoped key returns ITS OWN site's token regardless of the site we
    // asked for — using it would hydrate this app with another site's content.
    if (data.site && data.site !== (process.env.OLUX_SITE || 'v2-hairco')) {
      console.warn(`[olux] connect token belongs to site "${data.site}", not "${process.env.OLUX_SITE}" — tag omitted`)
      return ''
    }
    return data.token || ''
  } catch (e) {
...
export default defineNuxtConfig({
  ...
  runtimeConfig: {
    public: {
      ...
      cmsSite: process.env.OLUX_SITE || 'v2-hairco',
    },
  },
  app: {
    head: {
      title: 'Hair Co. Care — Salon & Hair Care',
      meta: [
        { charset: 'utf-8' },
      ...
      link: [
        ...
        { rel: 'stylesheet', href: 'https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,500;0,600;1,400&family=Jost:wght@300;400;500;600&family=Kaushan+Script&display=swap' },
        ...
        ? [
            {
              ...
              'data-site-name': process.env.OLUX_SITE || 'v2-hairco',
